...
Paykit status verification is worker-owned. The Lock Server sends canonical JSON `{ "creator": "pubky...", "bundle_id": "..." }` to `POST /transactions/status` with `X-Paykit-Signature` over those exact canonical body bytes. Valid response statuses are `undetected`, `detected`, `confirmed`, `cancelled`, and `expired`. Both terminal statuses require `confirmations = 0` and `amount_matched = false`: `cancelled` terminalizes the verification task as `cancelled`, while `expired` terminalizes it as `expired`; neither issues entitlement. Transport failures, timeouts, every non-2xx response (including `404` and authentication/authorization failures), unknown fields, noncanonical status tuples, and malformed success bodies are durably rescheduled as pending and are not retried again before the worker poll interval elapses.
