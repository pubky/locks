...
Creator publishing routes are authenticated and Pubky homeserver-backed. `PUT /creator/priv-resources/content/<path>`, `DELETE /creator/priv-resources/content/<path>`, `POST /creator/content-locks`, and `POST /creator/lock-service-config` require `Authorization: Bearer <frontend_session_token>`, derive creator from the Locks-local frontend session, and reject request-body creator spoofing. The raw guarded-resource upload body is bytes, not JSON/base64.
