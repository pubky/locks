...
Production Locks must use Pubky homeserver auth/capability semantics rather than inventing a parallel authorization model.
...
Current Pubky Homeserver code supports grant-backed app sessions: user-signed Grant JWS + Proof-of-Possession exchanged at `/auth/grant/session` for an opaque bearer. The SDK exposes `PubkyGrantAuthFlow`, `GrantCredential`, `PubkySession::from_grant_credential`, and export/import of durable grant secret material. Current SDK flow is deeplink + relay + homeserver exchange; a direct Locks callback endpoint would be additional product/API work rather than the shape currently dictated by `../pubky-homeserver/`.
...
- Locks relies on Pubky homeserver auth/capability semantics instead of inventing separate creator auth.
...
- Track SDK refresh/revalidation behavior as the Pubky SDK evolves, but keep retry policy lazy and single-attempt unless a concrete UX/operational need appears.
