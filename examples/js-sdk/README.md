...
```bash
curl -i http://127.0.0.1:15411
curl -i http://127.0.0.1:15412
```
...
If your local testnet homeserver differs, edit `./.local/demo-config/config.json` after `init-config` and change `testnet.homeserver`.
...
allowed_return_origins = ["http://127.0.0.1:8080"]
...
The Lock Server also needs its PKARR record published to the local relay. With the config above, startup/republishing should publish through `http://127.0.0.1:15411`. The browser SDK depends on that record when resolving `_pubky.<lock-server>`.
