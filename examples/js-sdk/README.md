...
2. Click **Submit proof bundle**. The browser submits one `paykit-payment` proof with the confirmed top-level `reader_public_key` and an empty `{}` criterion payload. It never calls the dev completion route. Under the submit control, the page displays the returned Noise connection state as **None — handshake has not started**, **Handshake in progress**, or **Connected — handshake completed**. While payment verification is pending, its existing lifecycle poll also repeats the exact idempotent proof submission to refresh this indicator without creating another verification task or invoice.
