...
5. The page polls `pending` and `in_progress` lifecycle states. On `completed`, it issues an access credential and reads the primary guarded resource. `failed`, `cancelled`, `expired`, and unknown states fail closed. Use **Resume payment verification polling** after a reload.
