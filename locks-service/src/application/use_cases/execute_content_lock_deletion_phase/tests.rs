...
async fn withdraw_exact_progresses_and_releases_guard() {
    let h = Harness::new();
    let outcome = h.execute(ContentLockDeletionPhase::Withdraw).await;
...
#[tokio::test]
async fn execute_matches_evidence_aware_outcome_for_every_phase() {
    for phase in [
        ...
        let outcome_harness = Harness::new();
        outcome_harness
            .tombstones
            .set_reads([TombstoneReadback::Exact; 4]);
        let outcome = outcome_harness.execute(phase).await;

        let evidence_harness = Harness::new();
        evidence_harness
            .tombstones
            .set_reads([TombstoneReadback::Exact; 4]);
        let evidence_outcome = evidence_harness.execute_with_evidence(phase).await.outcome;

        assert_eq!(outcome, evidence_outcome, "phase {phase:?}");
    }
}
...
async fn pubky_success_followed_by_repository_failure_preserves_source_independence() {
    ...
    *h.deletions.advance_error.lock().unwrap() = Some(ApplicationError::Storage {
    ...
    let execution = h
        .execute_with_evidence(ContentLockDeletionPhase::Withdraw)
        .await;
...
impl Harness {
    ...
    async fn execute(&self, phase: ContentLockDeletionPhase) -> DeletionPhaseExecutionOutcome {
        self.executor().execute(claim(phase), "worker").await
    }

    async fn execute_with_evidence(
        &self,
        phase: ContentLockDeletionPhase,
    ) -> DeletionPhaseExecution {
        self.executor()
            .execute_with_evidence(claim(phase), "worker")
            .await
    }
...
fn claim(phase: ContentLockDeletionPhase) -> ClaimedContentLockDeletionJob {
    claimed_deletion_job(NOW, phase, false)
}
