...
impl<'a> ContentLockDeletionPhaseExecutor<'a> {
    ...
    async fn execute_guarded_with_evidence(
        ...
        match claim.job.phase {
            ContentLockDeletionPhase::Withdraw => {
                self.withdraw_with_evidence(&claim, worker_id).await
            }
            ...
            ContentLockDeletionPhase::DrainExistingCredentials => self.repository_execution(
                self.advance(
                    &claim,
                    worker_id,
            ...
            ContentLockDeletionPhase::IssueFinalCredentials => {
                self.repository_execution(self.issue_final_credentials(&claim, worker_id).await)
            }
            ContentLockDeletionPhase::DrainFinalReads => self.repository_execution(
                self.advance(&claim, worker_id, ContentLockDeletionPhase::DeleteContent)
                    .await,
            ),
            ContentLockDeletionPhase::DeleteContent => {
                self.verify_frozen_content_with_evidence(&claim, worker_id)
                    .await
            }
            ContentLockDeletionPhase::DeleteTombstone => {
                self.verify_tombstone_for_purge_with_evidence(&claim, worker_id)
                    .await
            }
    ...
    async fn issue_final_credentials(
        ...
        match materialized {
            ...
            Ok(_) => {
                self.advance(claim, worker_id, ContentLockDeletionPhase::DrainFinalReads)
                    .await
            }
